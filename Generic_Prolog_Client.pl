...
create_client(Host, Port,Message) :-                                 %% create a Prolog client. Send request along with 'Message' to the Python server (Host:Port).
...
chat_to_server(In, Out,Message) :-                          
        ...
        (Term = end_of_file                                 %% If term = end_of_file then return
                ...
                flush_output(Out),
                read_string(In,L,X),                        %% read the data of Input stream in X. This data is sent by the Python server
                writeln(X)
        ),!.
